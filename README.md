...
- Components make code highly reusable (DRY)
...
- adding/changing functionality is easy as changes to the source code are not needed.
- lightweight as its executions are a series of asynchronous callbacks.

# APIs

## Backend API

This backend API allows the frontend application to interact with the backend database to display questions and results of the quiz as desired.
...
This app does not require any environment variables for it to work. Optionally you can add a .env file containing the following:
    ...
    *  SKIP_PREFLIGHT_CHECK=true - sometimes Create React App brings a preflight check error in which case set this variable to true
...
We used Cypress for the end-to-end testing to mimic user interactions. We used Jest react testing for the integration testing as Jest is better suited for testing the business logic of an application.
